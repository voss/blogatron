...
#		$post_head .= "<p class=\"date\">{$edate}</p>\n";
		...
		if(!empty($text_more))
		{
			$post_head .= "<p><a href=\"/{$date}/{$title_d}#mere\" style=\"font-weight: bold\" title=\"Læs mere af '{$title}'\">Læs mere...</a></p>";
		}
#		$post_head .= "</p>";
		print $post_head;
	...
	else
	{
		while($row = @mysql_fetch_array($result_year))
		{
			...
			while($row2 = @mysql_fetch_array($result_date))
			{
				...
				$date_u = $date_month;
				$date = dateify($date_month);
				print "<option value=\"{$install_path}/ee.php?arcmonth={$date_u}.{$year}\">".ucwords($date)."</option>\n";
			}
		}
	}
	print "</select>\n";
